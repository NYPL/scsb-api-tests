...
describe 'ItemAvailabilityStatus' do
  describe '3. Verify that when one or more items are looked up by barcode, each availability status is returned.', number:3 do
    [
      ...
      it "look up item availability by barcode #{query[:barcode]}" do
